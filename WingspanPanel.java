...
public class WingspanPanel extends JPanel implements MouseListener, MouseMotionListener {
    private final UIElement root, transition, startMenu, birdContainer;
    ...
    public WingspanPanel() {
        ...
        backgroundFrame.imagePath = "images/wingspan_background.png"; // setting the picture to the BufferedImage of the sky
        ...
        birdContainer = new UIFrame("BirdContainer", this); // invisible frame holding birdImage and birdHitbox (for petting)
        ...
        birdContainer.position = new Dim2(-0.1, 0, 0.45, 0); // slightly to the left of the start screen by 10% and centered in the middle vertically
        ...
        loadingTitle.size = new Dim2(0.25, 0, 0.2, 0);
        ...
        if (released != null) { // if we actually pressed and released something
            ...
            if (isStartButton != null && released.containsPoint(e.getX(), e.getY())) { // check if the mouse is still on the button and is actually a start button
                ...
                transition.tweenBackgroundTransparency(1f, 0.4, Tween.QUAD_IN_OUT).onFinish(() -> { // fade the transition in and then when it finishes
                    ...
                    startMenu.size = new Dim2(0.5, 0, 0.6, 0); // reset its size
        ...
        if (changed) {
            ...
            if (nowHovering != null) { // if we're hovering over something now
                ...
                if (hasDrop != null) {
                    ...
                    drop.tweenPosition(new Dim2(0.015, 0, 0.075, 0), 0.1, Tween.QUAD_IN_OUT); // move it down and right a little
                }
